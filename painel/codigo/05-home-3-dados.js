/* ============================================ OS CALCULOS DA ABA PAINEL
   GERADO A PARTIR DE `docs/propostas/painel/comum.js`, a proposta C aprovada
   por ele em 12/09/2026. Ordem numerica e lei.

   O TERRITORIO DESTA ABA:
   ...
   O DADO CHEGA DE `/painel/home`, numa resposta so'. Tudo aqui e' calculo em
   cima desse pacote: recortar por periodo, filtrar por etiqueta e somar. Nada
   volta ao servidor para trocar de periodo ou de filtro.

   PALAVRA SIMPLES em todo rotulo de tela: video guardado, video agendado, ja'
   publicado, pessoas alcancadas, dias que sobram, pasta de video.
   ========================================================================== */
(function () {
  var S = window.SALA;

  /* NA MAQUETE O PACOTE ERA UM ARQUIVO; AQUI ELE CHEGA DA REDE. `receber` troca
     tudo de uma vez e devolve o controle para a tela redesenhar: assim nao
     existe meio-termo em que metade da pagina mostra o dado novo e a outra
     metade o antigo. */
  var PERFIS = [], LEVAS = [], ESTEIRA = {}, RESUMO = {}, MAQUINA = [];
  var SAIDAS = [], FONTE = {}, ETIQUETAS = [], MERCADOS = [];
  var HOJE = new Date();
  function receber(d) {
    d = d || {};
    PERFIS = d.perfis || []; LEVAS = d.levas || []; ESTEIRA = d.esteira || {};
    RESUMO = d.resumo || {}; MAQUINA = d.maquina || []; SAIDAS = d.saidas || [];
    FONTE = d.fonte || {}; ETIQUETAS = d.etiquetas || [];
    MERCADOS = d.mercados || [];
    HOJE = new Date(d.hoje || new Date().toISOString().slice(0, 19));
    window.PN.PERFIS = PERFIS; window.PN.LEVAS = LEVAS;
    window.PN.ESTEIRA = ESTEIRA; window.PN.RESUMO = RESUMO;
    window.PN.MAQUINA = MAQUINA; window.PN.SAIDAS = SAIDAS;
    window.PN.FONTE = FONTE; window.PN.ETIQUETAS = ETIQUETAS;
    window.PN.MERCADOS = MERCADOS; window.PN.HOJE = HOJE;
  }
  ...
  /* A CASCA (tema, menu, marca) E' DO NUCLEO DO PAINEL, e nao desta aba: na
     maquete ela morava aqui porque a pagina era solta. */
