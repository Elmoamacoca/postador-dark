# -*- coding: utf-8 -*-
"""Publica as tres propostas da aba de CALENDARIO, uma por endereco.

Uma pagina por proposta, endereco fixo, e republicar no MESMO nome mantem o
endereco antigo valendo. O token sai de `.claude/secrets/vercel_token.txt`, que
guarda `NOME=valor`: mandar o arquivo inteiro devolve 403 sem explicar nada.

Rodar:  python docs/propostas/analytics/publicar.py
"""
import base64
import json
import pathlib
...
PROPOSTAS = {
    'borusa-calendario-a': 'proposta-a.html',
    'borusa-calendario-b': 'proposta-b.html',
    'borusa-calendario-c': 'proposta-c.html',
}
...
def main():
    ...
    for projeto, pagina in PROPOSTAS.items():
        try:
            ...
            print(projeto.ljust(20), 'ok  ', endereco, ident)
        except urllib.error.HTTPError as e:
            print(projeto.ljust(20), 'FALHOU', e.code, e.read()[:300].decode('utf-8',
                                                                             'ignore'))
            sys.exit(1)
